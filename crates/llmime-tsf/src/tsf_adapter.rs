...
impl TsfLiveAdapter {
    ...
    /// Synchronous wrapper for TSF callbacks; returns candidate strings or empty on error.
    pub fn handle_input_change(&self, input: &str) -> Vec<String> {
        ...
        match tokio::task::block_in_place(|| self.runtime.block_on(fut)) {
            ...
            Err(e) => {
                log::debug!("TsfLiveAdapter: on_input_change error: {e}");
                vec![]
            }
        }
    }
...
mod tests {
    ...
    fn make_adapter(consented: bool) -> (TsfLiveAdapter, tempfile::TempDir) {
        ...
        (TsfLiveAdapter::new(handler), dir)
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn handle_input_change() {
        let (adapter, _dir) = make_adapter(true);
        let result = tokio::task::spawn_blocking(move || adapter.handle_input_change("にほん"))
            .await
            .unwrap();
        assert_eq!(result, vec!["にほん"]);
    }
    ...
    async fn error_handling() {
        ...
        let handler = Arc::new(LiveConversionHandler::new(pipeline, cm));
        let adapter = TsfLiveAdapter::new(handler);
